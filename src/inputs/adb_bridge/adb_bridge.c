 ...
 *
 * Input format on the /dev/diag device: [ (int) USER_SPACE_DATA_TYPE constant ]
 * [ (int) MDM constant ] [ Raw data ] Output format on the /dev/diag device: [
 * (int) USER_SPACE_DATA_TYPE constant ] [ (int) MDM constant ] [ (int) Number
 * of buffers ] { [ (int) Buffer size ] [ Raw data ] .. }
 *
 * Related code from Android codebase:
 * - /dev/diag driver for Android 6.0
 * https://android.googlesource.com/kernel/msm.git/+/android-6.0.0_r0.9/drivers/char/diag
 *
 * Similar work:
 * - diag_revealer.c from MobileInsight
 * https://github.com/mobile-insight/mobileinsight-mobile/blob/master/diag_revealer/qcom/jni/diag_revealer.c
 * - diag-helper.c from SnoopSnitch
...
static void *(*real_pthread_create)(pthread_t *, const pthread_attr_t *,
                                    void *(*)(void *), void *) = NULL;
 ...
 * NEXUS-6-ONLY IOCTL
 * Reference:
 * https://github.com/MotorolaMobilityLLC/kernel-msm/blob/kitkat-4.4.4-release-victara/include/linux/diagchar.h
 */
 ...
 * Reference:
 * https://android.googlesource.com/kernel/msm.git/+/android-6.0.0_r0.9/drivers/char/diag/diag_dci.h
 */
 ...
 * Reference:
 * https://android.googlesource.com/kernel/msm.git/+/android-10.0.0_r0.87/drivers/char/diag/diagchar.h
 ...
 * Reference:
 * https://android.googlesource.com/kernel/msm.git/+/android-9.0.0_r0.31/drivers/char/diag/diagchar.h
 */
 ...
 * Reference:
 * https://android.googlesource.com/kernel/msm.git/+/android-7.1.0_r0.3/drivers/char/diag/diagchar.h
 */
...
#define DIAG_CON_ALL                                                           \
    (DIAG_CON_APSS | DIAG_CON_MPSS | DIAG_CON_LPASS | DIAG_CON_WCNSS |         \
     DIAG_CON_SENSORS)

/*
 * Structures for ioctl
 * Reference:
 * https://android.googlesource.com/kernel/msm.git/+/android-6.0.0_r0.9/drivers/char/diag/diagchar_core.c
 */
...
printf("\n");*/
        ...
        for (int i = 0; i < nb_msgs; i++) {
            if (send_buffer < diag_buffer ||
                send_buffer + (use_mdm ? 8 : 4) > diag_buffer + bytes_read) {
                goto wrong_length;
            }
...
/*
 * Explicitly probe the length of the argument that ioctl(diag_fd, req, ...)
 * takes.
 *
 * Assumptions:
 ...
 */
static ssize_t probe_ioctl_arglen(int req, size_t maxlen) {
    ...
    for (len = 0; len <= maxlen; ++len) {
        if (ioctl(diag_fd, req, p - len) >= 0) break;
 ...
 * in diag_switch_logging, three threads (disk_write_hdl,
 * qsr4_db_parser_thread_hdl, db_write_thread_hdl) will be created. But actually
 * we don't need them at all. Meanwhile we cannot call dlclose when these
 * useless threads are still alive. So the following fake pthread_create is used
 * to prevent them from being created.
 *
 * Note this fake pthread_create may cause some unexpected side effects on
 * another untested version of libdiag.so. If so, futher modification is needed.
 *
 * Tested devices:
...
static int __enable_logging_libdiag(int mode) {
    ...
    for (unsigned int i = 0;
         i < sizeof(LIB_DIAG_PATH) / sizeof(LIB_DIAG_PATH[0]) && !handle; ++i) {
    ...
    diag_switch_logging =
        (void (*)(int, const char *))dlsym(handle, "diag_switch_logging");
    if (!diag_switch_logging) goto fail;
    err = "diag_fd_ptr/fd";
    diag_fd_ptr = (int *)dlsym(handle, "diag_fd");
    if (!diag_fd_ptr) diag_fd_ptr = (int *)dlsym(handle, "fd");
    if (!diag_fd_ptr) goto fail;
...
static int enable_logging(int diag_fd, int mode) {
    int ret = -1;
    ...
    if (ret < 0) {
        printf(
            "ioctl DIAG_IOCTL_PERIPHERAL_BUF_CONFIG fails, with ret val = %d\n",
            ret);
     ...
     * Enable logging mode:
     *
     * DIAG_IOCTL_SWITCH_LOGGING has multiple versions. They require different
     * arguments (which have different fields and whose lengths are also
     * different). However, it seems there is no way to directly determine the
     * version of DIAG_IOCTL_SWITCH_LOGGING. So some tricks can not be avoided
     * here.
     *
     * A traditional way is to try one by one. But it can cause undefined
     * behaviour. Specially, when a new verison of DIAG_IOCTL_SWITCH_LOGGING is
     * introduced, it may not report an error. But some new fields will be out
     * of bounds. Consequently, it may cause random bugs, which is confusing.
     *
     * So a more elegant way is to explicitly probe the length of
     * DIAG_IOCTL_SWITCH_LOGGING's argument. And the version can be deduced from
     * the length. It is not very precise, but it is enough at least for now.
     */
    ...
    long arglen = probe_ioctl_arglen(
        DIAG_IOCTL_SWITCH_LOGGING, sizeof(struct diag_logging_mode_param_t_q));

    printf("system_version: %s\n", system_version);

    int system_version_num = atoi(system_version);

    if (

        strstr(board_name, "lito") != NULL && (system_version_num > 10)) {
        // printf("MATCHED.\n");
        /* Android 11.0.0 (RD1A.201105.003.C1)
         * Reference:
    ...
    case sizeof(struct diag_logging_mode_param_t): {
        /* Android 7.0 mode
         * Reference:
         * https://android.googlesource.com/kernel/msm.git/+/android-7.1.0_r0.3/drivers/char/diag/diagchar_core.c
         */
    ...
    case sizeof(int):
        ...
        if (ret >= 0) break;
        /*
         * Is it really necessary? It seems that the kernel will simply ignore
         * all the fourth and subsequent arguments of ioctl. But similar lines
         * do exist in libdiag.so. Why? Reference:
         * https://android.googlesource.com/kernel/msm.git/+/android-10.0.0_r0.87/fs/ioctl.c#692
         */
    ...
    case 0:
        ...
        ret = ioctl(diag_fd, DIAG_IOCTL_SWITCH_LOGGING, (long)mode, 12, 0, 0, 0,
                    0);
        if (ret >= 0) break;
    default:
        LOGW("ioctl DIAG_IOCTL_SWITCH_LOGGING with arglen=%ld is not "
             "supported\n",
             arglen);
        ret = -8080;
        break;
...
next:
    ...
    //    arglen); printf("ioctl DIAG_IOCTL_SWITCH_LOGGING with arglen=%ld
    //    succeeded\n", arglen);
    ...
    if (ret >= 0) {
        ...
        ret = ioctl(diag_fd, DIAG_IOCTL_PERIPHERAL_BUF_CONFIG, &buffering_mode);
        if (ret < 0) {
            // LOGD("ioctl DIAG_IOCTL_PERIPHERAL_BUF_CONFIG fails, with ret val
            // = %d\n", ret); perror("ioctl DIAG_IOCTL_PERIPHERAL_BUF_CONFIG");
        }
...
int main(void) {
    ...
    *(unsigned int *)diag_buffer = USER_SPACE_DATA_TYPE;
    ...
    return_value = setsockopt(server, SOL_SOCKET, SO_REUSEADDR, &DO_REUSE_ADDR,
                              sizeof DO_REUSE_ADDR);
    ...
    while (1) {
        ...
        for (int i = 0; i < number_fds; i++) {
            ...
            if (some_fd.fd == server) {
                ...
                int client = accept(server, (struct sockaddr *)&client_info,
                                    &client_info_size);
                ...
                int bytes_read =
                    read(some_fd.fd, diag_buffer + (use_mdm ? 8 : 4),
                         BUFFER_LEN - (use_mdm ? 8 : 4));
                if (bytes_read < 1) {
                    goto remove_fd;
                }

                return_value =
                    write(diag_fd, diag_buffer, bytes_read + (use_mdm ? 8 : 4));
                if (return_value < 0) {
                    if (errno != EFAULT) {
                        error("write");
                    } else {
#ifdef DEBUG
                        printf("Note: EFAULT was encountered while writing "
                               "message\n");
#endif
            ...
            remove_fd:
                ...
                memcpy(&fds[i], &fds[i + 1],
                       sizeof(fds[0]) * (FDS_LEN - number_fds));
                number_fds--;
